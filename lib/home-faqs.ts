...
export function homeFaqsForBlocks(blocks: Record<string, ContentBlock>) {
  return HOME_FAQ_ITEMS.map((item) => ({
    ...item,
    q: pickTextOrUndef(blocks, `home.faq.item${item.idx}.q`) ?? item.q,
    a: pickTextOrUndef(blocks, `home.faq.item${item.idx}.a`) ?? item.a,
  }))
}
