...
function ImageFrame({
  ...
  if (!imageUrl) {
    ...
    return (
      <div className="flex min-h-[220px] items-center justify-center rounded-lg border border-dashed border-ink-200 bg-ink-50 px-6 text-center text-sm font-bold text-ink-400">
  ...
  return (
    <figure>
      <img
        ...
        className="w-full rounded-lg border border-ink-100 bg-ink-50 object-cover shadow-sm"
        loading="lazy"
      />
...
export function LandingModuleRenderer({ item, showPlaceholders = false }: Props) {
  ...
  if (item.item_type === 'split') {
    ...
    return (
      <section {...moduleAttrs(item)} className="bg-ink-50 py-section">
        <div
          ...
          <div className="grid gap-5 md:grid-cols-3">
            {cards.map((card, index) => (
              <article
                ...
                className="rounded-lg border border-ink-150 bg-white p-6 shadow-sm transition-all hover:-translate-y-0.5 hover:border-brand-blue/40 hover:shadow-md md:p-7"
              >
